...
_STATES: dict[ObjectKind, tuple[str, ...]] = {
    # v0.0 only uses states that can be depicted as ordinary physical geometry.
    # Briefcase latch and lamp power glyphs are intentionally excluded from the
    # benchmark state vocabulary because they required benchmark-specific symbol
    # conventions during blind review.
...
# Object kinds eligible to anchor STATE and CONFLICT cases.
...
def _sample_position(rng: random.Random, size: Size) -> Point:
    """Sample a position while keeping the object's box inside the canvas."""
...
def _overlaps(candidate: Point, size: Size, objects: list[SceneObject], *, gap: float = 0.025) -> bool:
    """Return True when a candidate box overlaps an existing object plus a small gap."""
...
def generate_scene(seed: int, *, scene_id: str | None = None) -> Scene:
    ...
    Design guarantees for v0.0:
    - exactly six objects (comfortably within the 4-8 object guardrail),
    - at least one explicit spatial relation,
    - one unverified witness statement that contradicts a physical state,
    - a physical-over-testimony rule,
    - no model calls or external randomness.
    """
    ...
    # Ensure the scene has at least one state-bearing object for conflict cases,
    # then fill remaining slots without replacement for visual variety.
    ...
    # Pick a relation whose direction can be made consistent with the geometry.
    subject, reference = rng.sample(objects, 2)
    if subject.position.x <= reference.position.x:
        relation_kind = RelationKind.LEFT_OF
    else:
        relation_kind = RelationKind.RIGHT_OF
    relation = SpatialRelation(subject.object_id, relation_kind, reference.object_id)

    anchor = objects[0]
    assert anchor.state is not None
    contradictory_state = _alternate_state(anchor.kind, anchor.state)
    statement = WitnessStatement(
        statement_id="stmt_1",
        witness_id="witness_a",
        claim=(
            f"The witness says the {anchor.color} {anchor.kind.value} "
            f"was {contradictory_state}."
        ),
        subject_id=anchor.object_id,
        predicate="state",
        value=contradictory_state,
        verified=False,
    )
    ...
    if seed % 2 == 0:
        rule = CaseRule(
            rule_id="rule_1",
            description="Current physical evidence overrides unverified witness testimony.",
        ...
        rule = CaseRule(
            rule_id="rule_1",
            description=(
                "When physical evidence and unverified testimony disagree, "
                "neither source has automatic priority."
            ),
            evidence_priority=EvidencePriority.NO_PRIORITY,
        )
